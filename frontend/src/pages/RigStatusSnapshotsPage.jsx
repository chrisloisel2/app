...
import EntitySelect, { projectItems, rigItems } from "../components/ui/EntitySelect";
...
export default function RigStatusSnapshotsPage() {
  const { projects, rigs } = useReferenceData();
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        {(() => {
          ...
          return (<>
            <select value={filters.rig_id} onChange={(e) => setFilter("rig_id", e.target.value)} className={cls}>
              ...
              <tbody className="divide-y divide-gray-100">
                {data.items.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 font-semibold text-gray-900">{rigs.find((r) => r._id === item.rig_id)?.code ?? item.rig_id}</td>
                    <td className="px-4 py-2 text-gray-600">{projects.find((p) => p._id === item.project_id)?.code ?? item.project_id}</td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Rig <span className="text-red-500">*</span></label>
                  <EntitySelect value={form.rig_id} onChange={(v) => set("rig_id", v ?? "")}
                    items={rigItems(rigs)} placeholder="— Sélectionner un rig —" required />
                </div>
                <div>
                  ...
                  <EntitySelect value={form.project_id} onChange={(v) => set("project_id", v ?? "")}
                    items={projectItems(projects)} placeholder="— Sélectionner un projet —" required />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
