...
export default function AnnotateursPage() {
  ...
  const openEdit = (a) => {
    setEditTarget(a);
    setForm({ numero_poste: a.numero_poste, nom_utilisateur: a.nom_utilisateur, mdp: "" });
    setFormError(null);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editTarget) {
        await updateAnnotateur(editTarget._id, form);
      } else {
        await createAnnotateur(form);
      }
      setModalOpen(false);
      load();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAnnotateur(deleteTarget._id);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Annotateurs</h1>
          <p className="text-sm text-gray-500 mt-0.5">Gestion des comptes annotateurs</p>
        </div>
        <button
          onClick={openCreate}
            ...
            <tbody className="divide-y divide-gray-100">
              {annotateurs.map((a) => (
                <tr key={a._id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-gray-700">{a.numero_poste}</td>
                  <td className="px-4 py-3 text-gray-900 font-medium">{a.nom_utilisateur}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-5">
            <h2 className="text-lg font-bold text-gray-900">
              {editTarget ? "Modifier l'annotateur" : "Nouvel annotateur"}
            </h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6 space-y-4">
            ...
            <p className="text-sm text-gray-600">
              Supprimer l'annotateur <span className="font-semibold">{deleteTarget.nom_utilisateur}</span> ? Cette action est irréversible.
            </p>
